...
import { Plus, Trash2, HeartHandshake, Pencil } from "lucide-react"
import EntryEditor from "./EntryEditor"
import { VolunteerStepSchema, type VolunteerEntry } from "@/lib/resume/schema"
...
export default function VolunteerStep({ entries, onChange, optional = false }: VolunteerStepProps) {
  const [isEditing, setIsEditing] = useState(false)
  const [editingIndex, setEditingIndex] = useState<number | null>(null)

  const errors = useMemo(() => {
    const result = VolunteerStepSchema.safeParse({ entries })
    return result.success ? {} : result.error.flatten().fieldErrors
  }, [entries])

  const handleSave = (entry: VolunteerEntry) => {
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-neutral-600">{entry.subtitle} · {entry.location}</p>
      ...
      {optional && entries.length === 0 && (
        <p className="text-sm text-neutral-500">No volunteer experience added. You can skip this section.</p>
      )}
    </div>
  )
}
